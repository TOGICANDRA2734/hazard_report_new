	              ...
	              <?= form_error('isi_laporan'); ?>
						...
						<div class="form-group">
							...
							<select class="form-control" id="form_kecamatan">
								<?php 
									$getKecamatanByIdFromKelurahan = $this->db->get_where('site', ['kodesite' => $pengaduan['id_kecamatan']])->row_array();
								?>
								<option value="<?= $getKecamatanByIdFromKelurahan['kodesite']; ?>"><?= $getKecamatanByIdFromKelurahan['namasite']; ?></option>
								<?php foreach ($kecamatan as $dataKecamatan): ?>
									<?php if ($dataKecamatan['kodesite'] != $getKecamatanByIdFromKelurahan['kodesite']): ?>
										<option value="<?= $dataKecamatan['kodesite']; ?>"><?= $dataKecamatan['namasite']; ?></option>
									<?php endif ?>
								<?php endforeach ?>
							</select>
						</div>
						<div class="form-group">
							...
							<select id="form_kelurahan" class="form-control" name="id_kelurahan">
								<?php 
									$getKelurahanByIdKecamatan = $this->db->get_where('pma_dailyprod_pit', ['kodesite' => $dataKecamatan['id_kecamatan']])->result_array();
								?>
								<option value="<?= $pengaduan['id']; ?>"><?= $pengaduan['ket']; ?></option>
								<?php foreach ($getKelurahanByIdKecamatan as $dataKelurahan): ?>
									<?php if ($dataKelurahan['id_pit'] != $pengaduan['id_pit']): ?>
										<option value="<?= $dataKelurahan['id_pit']; ?>"><?= $dataKelurahan['ket']; ?></option>
									<?php endif ?>
								<?php endforeach ?>
							</select>
						</div>
