	...
	<div class="container mb-5">
		<div class="row justify-content-center py-3">
			<div class="col-lg header-title">
				<h3 class="text-center"><?= $pengaduan['isi']; ?></h3>
			</div>
		</div>
			...
			<div class="col-lg">
				<table>
					<tr>
						...
						<td class="align-middle"><?= $pengaduan['isi']; ?></td>
					</tr>
					<tr>
						<th class="align-middle">Tanggal Pengaduan</th>
						<td style="width: 3rem" class="align-middle text-center">:</td>
						<td class="align-middle"><?= date('d-M-Y, \P\u\k\u\l H:i', strtotime($pengaduan['tgl'])); ?></td>
					</tr>
					<tr>
						<th class="align-middle">Lokasi</th>
						<td style="width: 3rem" class="align-middle text-center">:</td>
						<td class="align-middle"><?= $pengaduan['kodepit']; ?></td>
					</tr>
				</table>
			</div>
		</div>
						...
						<tbody>
							...
							<?php foreach ($tanggapan as $dt): ?>
								<tr>
									...
									<td class="align-middle"><?= date('d-M-Y, \P\u\k\u\l H:i', strtotime($dt['tgl'])); ?></td>
									<td class="align-middle"><?= $dt['isi']; ?></td>
									<?php 
										$status = explode('_', $dt['status_pengaduan']);
										$status = implode(' ', $status);
										$status = ucwords(strtolower($status));
									?>
									<td class="align-middle text-center">
										<a href="<?= base_url('assets/img/img_tanggapan/') . $dt['foto']; ?>" class="enlarge">
											<img src="<?= base_url('assets/img/img_tanggapan/') . $dt['foto']; ?>" class="img-fluid img-w-75-hm-100" alt="<?= $dt['foto']; ?>">
										</a>
									</td>
									<td class="align-middle">
										<?php if ($dt['status_pengaduan'] == 'proses'): ?>
											<button type="button" class="btn btn-sm text-center btn-danger"><i class="fas fa-fw fa-sync"></i> <?= $status; ?></button>
										<?php elseif ($dt['status_pengaduan'] == 'valid'): ?>
											<button type="button" class="btn btn-sm text-center btn-success"><i class="fas fa-fw fa-check"></i> <?= $status; ?></button>
										<?php elseif ($dt['status_pengaduan'] == 'pengerjaan'): ?>
											<button type="button" class="btn btn-sm text-center btn-warning"><i class="fas fa-fw fa-hammer"></i> <?= $status; ?></button>
										<?php elseif ($dt['status_pengaduan'] == 'selesai'): ?>
											<button type="button" class="btn btn-sm text-center btn-primary"><i class="fas fa-fw fa-check-double"></i> <?= $status; ?></button>
										<?php elseif ($dt['status_pengaduan'] == 'tidak_valid'): ?>
