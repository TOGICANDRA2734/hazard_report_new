...
class Tanggapan_model extends CI_Model 
{
	...
	{
		$this->db->join('user', 'hazard_tanggapan.id_user=user.id');
		$this->db->join('pengaduan', 'hazard_tanggapan.id_pengaduan=hazard_pengaduan.id_pengaduan');
		$this->db->order_by('id', 'asc');
		return $this->db->get('hazard_tanggapan')->result_array();
	}

	public function getTanggapanByIdPengaduan($id_pengaduan)
	{
		$this->db->join('user', 'hazard_tanggapan.id_user=user.id');
		$this->db->join('hazard_pengaduan', 'hazard_tanggapan.id_pengaduan=hazard_pengaduan.id_pengaduan');
		$this->db->order_by('hazard_tanggapan.id', 'asc');
		return $this->db->get_where('hazard_tanggapan', ['hazard_pengaduan.id_pengaduan' => $id_pengaduan])->result_array();
	}
	...
	public function getTanggapanGroupByIdPengaduan($status_tanggapan = '')
	{
		...
		$this->db->join('user', 'hazard_tanggapan.id_user=user.id');
		$this->db->join('pengaduan', 'hazard_tanggapan.id_pengaduan=pengaduan.id_pengaduan');
		$this->db->order_by('hazard_tanggapan.tgl_tanggapan', 'desc');
		return $this->db->get('hazard_tanggapan')->result_array();
	}
	...
	public function getTanggapanById($id_tanggapan)
	{
		$this->db->join('user', 'hazard_tanggapan.id_user=user.id');
		$this->db->join('pengaduan', 'hazard_tanggapan.id_pengaduan=hazard_pengaduan.id_pengaduan');
		return $this->db->get_where('hazard_tanggapan', ['id_tanggapan' => $id_tanggapan])->row_array();	
	}
	...
	public function addTanggapan($id_pengaduan)
	{
		...
		$this->db->update('hazard_pengaduan', ['status_pengaduan' => $status_tanggapan], ['id_pengaduan' => $id_pengaduan]);
