...
class PelaporPengaduan extends CI_Controller 
{
	...
	public function index($status_pengaduan = '')
	{
		...
		$data['pengaduan']	= $this->pepemo->getPengaduanByStatusPengaduanByIdMasyarakat($status_pengaduan, $data['dataUser']['id']);
	...
	public function editPelaporPengaduan($id_pengaduan)
	{
		...
		$data['title'] 		= 'Ubah Pengaduan - ' . $data['pengaduan']['isi'];
	...
	public function removePelaporPengaduan($id_pengaduan)
	{
		...
		// cek status pengaduan
		if ($tanggapan = $this->db->get_where('hazard_tanggapan', ['id_pengaduan' => $id_pengaduan])->row_array()) 
		{
			if ($tanggapan['status'] != null) 
			{
